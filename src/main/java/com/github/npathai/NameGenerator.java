...
public class NameGenerator {
    ...
    static final List<String> SURNAMES = Arrays.asList(
     ...
     * Generates a random name from the list of adjectives and surnames formatted as "adjective_surname".
     * For example 'focused_turing'.
     * If retry is true, a random integer between 0 and 10 will be added to the end of the name,
     * e.g `focused_turing3`
     */
    public String generateRandomName(boolean retry) {
        String name;
        do {
            name = String.format("%s_%s", ADJECTIVES.get(random.nextInt(ADJECTIVES.size())),
                    SURNAMES.get(random.nextInt(SURNAMES.size())));
        } while (name.equals("boring_wozniak"));
